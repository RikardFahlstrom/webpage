...
class EmailService:
    ...
    __smtp_server = None
    __smtp_port = None
    __is_debug_mode = True
    __from_address = 'rikard.email.notification@gmail.com'
    ...
    def send_welcome_email(cls, email):
        ...
        EmailService.send_email(email, 'Welcome!', html_body)

    @classmethod
    def send_password_reset_email(cls, email, reset_id):
        html_body = EmailTemplateParser.expand(
            EmailTemplateParser.password_reset,
            {'reset_code': reset_id}
        )

        EmailService.send_email(email, 'Reset your password at rikardfahlstrom.com', html_body)